...
> El costo del producto al momento de la venta se guarda automáticamente en la transacción (no te lo pide la UI). Esto permite que los reportes de margen sean exactos aunque después cambie el costo en el catálogo.
...
### Editar precios de una transacción ya guardada

Al editar una transacción, el **Precio venta** (o **Costo unitario** en compras y traslados) aparece **con el valor que tenía cuando se guardó la transacción** (no con el valor actual del catálogo). Si lo modificas, se reemplaza; si no lo tocas, queda igual.

En ventas, el costo de adquisición no se muestra en la UI pero queda guardado como snapshot histórico del momento de la venta.
